...
// hacky way to ensure compatability with pillow without using c++2a
// only works on gcc last i checked
#define DEF_OR_ARG(useless,in,precision,...) precision
#define clip8(in,...) CLIP8V2(in, DEF_OR_ARG(,##__VA_ARGS__,__VA_ARGS__,PRECISION_BITS))

static inline UINT8 CLIP8V2(int in, int precision)
{
